...
def train_model(X,y,params,model,scale=False,nfolds=10,n_jobs=15,cv = None,n_params=100):
	"""
...
def train_func(sets,
	...
	model,preds = train_model(sets.drop(["time","IDENTIFIER","system"],axis=1, errors='ignore'),
					  					   sets["time"],params,model,
					  					   cv = cv,n_params=n_params,n_jobs=15)
	...
	model,preds = train_model(sets.drop(["time","IDENTIFIER","system"],axis=1, errors='ignore'),
					  					   sets["time"],params,model,
					  					   n_jobs=15,cv = cv,n_params=n_params)
